pipeline {
    ...
    environment {
        ...
        CONTAINER_NAME = "speingboot-container" //도커 컨테이너 이름
        JAR_FILE_NAME = "app.jar" //복사한 JAR 파일 이름
        PORT="8081" //컨테이너와 연결할 포트

        REMOTE_USER = "ec2-user" //원격(spring) 서버 사용자
        REMOTE_HOST = "3.37.86.129" //원격(spring) 서버 IP (public IP)

        REMOTE_DIR = "/home/ec2-user/deploy" //원격 서버에 파일 복사할 경로
        SSH_CREDENTIALS_ID = "c1a4684d-fe42-4850-8c38-c905b30cd9f9" //Jenkins ssh 자격증명 ID
    }
